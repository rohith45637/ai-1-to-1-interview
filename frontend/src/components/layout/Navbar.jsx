...
export function Navbar({ activeTab, setActiveTab }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-surface-800/80 bg-surface-950/80 backdrop-blur-xl transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          >
            ...
            <div>
              <span className="font-bold text-base tracking-tight text-white flex items-center gap-2">
                1-to-1 Interview
                <span className="text-[10px] uppercase font-extrabold tracking-wider bg-brand-500/15 text-brand-400 border border-brand-500/30 px-1.5 py-0.5 rounded-full">
                  AI Pro
                </span>
              </span>
              <p className="text-[11px] text-surface-400 hidden sm:block">Intelligent AI Career Assessment Platform</p>
            </div>
          </div>

          {/* Desktop Navigation Links */}
          <nav className="hidden md:flex items-center gap-1 bg-surface-900/90 p-1.5 rounded-2xl border border-surface-800 shadow-inner">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge && !isActive && (
                    <span className="text-[9px] px-1.5 py-0.2 rounded-full bg-brand-500/15 text-brand-300 border border-brand-500/20 font-bold uppercase">
                      {item.badge}
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center gap-2.5">
            <div className="flex items-center gap-1.5 bg-amber-500/10 text-amber-300 border border-amber-500/30 px-3 py-1.5 rounded-full text-xs font-bold shadow-sm">
            ...
            <button
              ...
              className="p-2 rounded-xl border border-surface-800 bg-surface-900/80 text-surface-300 hover:text-white hover:bg-surface-800 transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className="md:hidden p-2 rounded-xl border border-surface-800 bg-surface-900 text-surface-300 hover:text-white cursor-pointer"
            >
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-surface-800 bg-surface-950/95 backdrop-blur-xl px-4 pt-3 pb-5 space-y-1.5 shadow-2xl animate-in slide-in-from-top-2 duration-150">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeTab === item.id;
            return (
